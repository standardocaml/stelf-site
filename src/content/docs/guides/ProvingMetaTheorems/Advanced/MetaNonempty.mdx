...
> For all terms $ e' $ and $ e $, $ |e| \le |\{e'/x\}e| $.

We can restate this theorem relationally as follows:

> For all terms $ e' $ and $ e $, if $ |e| = n $ and $ |\{e'/x\}e| = n' $ then there exists an $ n'' $ such that $ n'' + n = n' $.
...
Considering arbitrary contexts in the world ``(size-block)*`` is necessary to capture the informal theorem statement, which ranges over all possibly-open terms $ e $ and $ e' $.  The ``size`` derivation for the term $ e $ with a distinguished free variable $ x $ is represented by an LF term of higher type ``{x:tm} size x (s z) -> size (E x) N``.  
...
Putting a case of a theorem in the context is a general technique that is useful in many circumstances.  However, in this example, we can avoid putting the theorem case in the context by using a [catch-all case](/wiki/catch-all-case/), as the tutorial on that STELF proof device explains.  
