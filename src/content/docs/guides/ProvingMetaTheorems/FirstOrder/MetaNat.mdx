...
The reason is that we can use STELF's ability to _prove_ totality assertions to _check_ proofs of more general metatheorems.  Let's illustrate this with an example.  Say we want to prove that the sum of two even numbers is even.  More precisely, let's prove the following statement:

> If $ \mathsf{even}(N_1) $ and $ \mathsf{even}(N_2) $ and $ \mathsf{plus}(N_1,N_2,N_3) $ then $ \mathsf{even}(N_3) $.

We can prove this statement by rule induction on the first premise, the derivation that $ \mathsf{even}(N_1) $.  The computational content of this proof will be a function that transforms the derivations of the premises into a derivation of the conclusion.  
...
The proof is by induction over the derivation of $ \mathsf{even}(N_1) $.  

Case for $ \overline{\mathsf{even}(\mathsf{zero})} $:

In this case, $ N_1 $ is $ \mathsf{zero} $, so by inversion the derivation of $ \mathsf{plus}(N_1,N_2,N_3) $ must derive $ \mathsf{plus}(\mathsf{zero},N_2,N_2) $.  Then the assumption of $ \mathsf{even}(N_2) $ gives the result.  

Case for $ \mathsf{even}(N_1’) \over \mathsf{even}(\mathsf{succ}(\mathsf{succ}(N_1’))) $:

In this case, $ N_1 $ is $ \mathsf{succ}(\mathsf{succ}(N_1’)) $, so we have a derivation of $ \mathsf{plus}(\mathsf{succ}(\mathsf{succ}(N_1’)),N_2,N_3) $.  By inversion on this derivation, $ N_3 $ is $ \mathsf{succ}(\mathsf{succ}(N_3’)) $ and we have a derivation of $ \mathsf{plus}(N_1’,N_2,N_3’) $.  Then the inductive hypothesis applied to this derivation and the derivation of $ \mathsf{even}(N_2) $ yields a derivation of $ \mathsf{even}(N_3’) $.  So we can derive 
...
This means that there are many theorems that cannot be mechanized in STELF (e.g., theorems requiring higher quantifier complexity).  However, in practice, $ \forall\exists $-statements have been shown to be a useful class of metatheorems about programming languages (for example, they are enough to [prove type safety for all of Standard ML](https://dl.acm.org/doi/10.1145/1190215.1190245)).    

It is important to recognize that the restriction to $ \forall\exists $-statements is a consequence of  representing proofs of metatheorems as total relations among LF types.  It is _not_ a consequence of representing object languages in LF.  Any on-paper proof about an informal description of an object language can be translated into an on-paper proof about the associated adequate LF representation.  But only some of these proofs can be mechanically verified in STELF using the techniques presented above.  
