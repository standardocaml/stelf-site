...
The judgement $ \mathsf{even}(n) $ holds when $ n $ is even.  It is inductively defined by the following inference rules:
...
The second term constant ``even-s``, corresponds to the second inference rule above, which, for any $ n $, constructs a derivation of $ \mathsf{even}(\mathsf{succ}(\mathsf{succ}(n))) $ from a derivation of $ \mathsf{even}(n) $.  To encode this inference rule, the constant ``even-s`` is given a [dependent function type](/wiki/dependent-types/).  
...
```
 ...
 The dependent function type is used to bind the variable ``N`` in the type ``(even N -> even (s (s N)))``, expressing that the inference rule is schematic in $ n $.

When a term of dependent function type is applied to an argument, the argument is substituted into the result type.  For example, the term ``even-s z`` has type ``even z -> even (s (s z))``—i.e., it is the specialization of the inference rule to $ n = \mathsf{zero} $.  That looks like this: 
...
To check that this LF representation is correct ([adequate](/wiki/adequacy/)), we must verify that there is a bijection between the derivations of $ \mathsf{even}(n) $ and the LF terms of type ``even N`` where $ n \gg N $.  In particular, this LF signature adequately represents natural numbers and derivations of $ \mathsf{even}(n) $ when considered in the empty LF context. It would not be adequate in contexts containing assumptions of the form ``x : nat`` (which don't correspond to any natural number) or ``d : even N`` (which don't correspond to any of the rules for deriving $ \mathsf{even}(n) $).
...
```
 The type family ``plus`` is indexed by three terms of type ``nat`` because the informal judgement has three parameters.  The constants correspond to the two inference rules, inhabiting the type family with terms representing derivations.  For example, this term represents a derivation that $ 2+1 = 3 $. 
...
To check correctness of this representation, we must verify that there is a bijection between derivations of $ \mathsf{plus}(n_1,n_2,n_3) $ and LF terms of type
``plus N1 N2 N3`` where $ n_i \gg N_i $.  Once again, the definition of the encoding judgement and the adequacy proof are simple.
