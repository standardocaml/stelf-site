...
Here is a complete STELF proof of preservation for the STLC: 
```stelf 
...
We should check that this STELF theorem statement actually corresponds to the notion of type preservation that we have in mind, which is stated as follows:
...
The above totality assertion for the type family ``preserv`` clearly implies this statement.  Thus, the STELF proof proves the informal statement of preservation.
...
The ``%total`` directive asks STELF to verify that ``preserv`` defines a total relation—i.e., that it is a correct proof.  We call attention to a few of STELF's checks:
...
* **Input coverage:** The inversion steps in the above reading of the proof create input-coverage-checking obligations. For example, STELF's coverage checker justifies the inversion of the typing derivation in ``preserv-app-1`` by using index information (i.e., that the subject of the derivation is ``app E1 E2``) to show that ``of-app`` is the only way to construct an LF term of the appropriate type.  If there were another way to derive the well-typedness of an application, the inversion of the typing derivation in ``step-app-1`` would not be justified.  This would manifest itself as an input coverage error, as not all possible inputs would be derived by ``of-app``.

## Other theorems

Other properties of the STLC are proved elsewhere on this wiki.  Several [tutorials](/wiki/tutorials/) use theorems about the STLC to illustrate various STELF devices.  For example, proving progress for the STLC uses [output factoring](/wiki/output-factoring/); proving determinacy of the operational semantics is an example of a [uniqueness lemma](/wiki/uniqueness-lemma/).  You may wish to read these tutorials at this point.  Neither requires any more machinery than what we have already introduced; the tutorials simply show how to use STELF in a fashion that may be somewhat subtle until you've seen it once.

The next section of this introduction describes one new piece of STELF machinery: the ability to prove totality assertions for non-empty contexts.
