...
We consider a λ-calculus with $ \mathsf{unit} $ as a base type:
...
arrow : tp -> tp -> tp.
 ...
 We leave it to the reader to formulate an appropriate encoding judgement between STLC types $ \tau $ and closed LF terms of type ``tp`` and to prove that this encoding is adequate in the usual sense.  

## Representing STLC terms

### Informal definition

The terms of the STLC are variables $ x $, the 0-element tuple $ \langle\rangle $, lambda abstraction (with a type annotation), and application:

$$ 
e ::= x \mid \langle\rangle \mid \lambda x{:}\tau.e \mid e_1\,e_2
 $$

The variable $ x $ is considered to be bound by the expression $ \lambda x{:}\tau.e $.  Following standard practice, we implicitly consider terms modulo renaming of bound variables.
...
lam   : tp -> (tm -> tm) -> tm.
 ...
 The representation of $ \langle\rangle $ as ``empty`` and application as ``app`` is no different than what we did when representing natural numbers.  Two questions you are may be asking are:

1. How does this LF signature represent variables $ x $, since these don't appear in the LF signature, and
...
The answer is that we are using a representation technique called _higher-order abstract syntax_ (HOAS): we use LF variables to represent object-language variables.  Here are some example representations: 
 ...
 - The term $ \lambda x{:}{\mathsf{unit}}. x $ 
   is represented in LF as `lam unit ([x] x)`.
 - The term $ \lambda x{:}{\mathsf{unit} \rightarrow \mathsf{unit}}. x\,\langle\rangle $ is represented in LF as `lam (arrow unit unit) ([x] app x empty)`


As you can see, we represent an object-language variable by an LF variable with the same name.  To represent an object-language term $ \lambda x {:}\tau . e $, we first represent the body $ e $ as an LF term ``M`` in which the LF variable ``x`` occurs free.  Then, we bind this variable using an LF abstraction ``[x] M``.  This abstraction, which represents the body of the object-language lambda, has LF type ``tm -> tm``.  That's where the higher-order type of the constant ``lam`` comes from.
...
* **capture-avoiding substitution**: an object-language substitution, written $ \{e_2/x\}e $, is modeled by an LF substitution ``[E2/x] E``.  
...
There is a bijection $ e \gg \mathtt{E} $ $ e \gg \texttt{E} $ between (α-equivalence classes of) STLC terms $ e $ with free variables in $ x_1,\ldots,x_n $ and (α-equivalence classes of) LF terms ``E : tm`` in the LF context ``x1:tm, ..., xn:tm``.  

Moreover, this bijection is compositional in the following sense: if $ e \gg \mathtt{E} $ and $ e' \gg \texttt{E'} $ then $ \{e'/x\}e \gg \texttt{[E'/x] E} $.  
...
**Adequacy for terms requires adequacy for types.**  Because the syntax of terms mentions the syntax of types, the proof of adequacy for terms requires knowing that ``tp`` adequately represents types $ \tau $.  Above, we (well, we left it as an exercise for you) proved that types$ \tau $ are adequately represented by _closed_ LF terms of type ``tp``.  Thus, there is a danger here: the syntax of STLC types might not be adequately represented in the LF contexts we consider here, which have the form ``x1:tm, ..., xn:tm``.  
